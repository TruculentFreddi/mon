...
cat > /home/postgres/project_ha/env/${1}_keepalived.conf << EOF
vrrp_instance VI_1 {
    state MASTER
    interface $NET_INT
...
cat > /home/postgres/project_ha/env/${2}_keepalived.conf << EOF
vrrp_instance VI_1 {
    state BACKUP
    interface $NET_INT
    virtual_router_id 33
...
cat > /home/postgres/project_ha/env/${3}_keepalived.conf << EOF
vrrp_instance VI_1 {
    state BACKUP
    interface $NET_INT
    virtual_router_id 33
